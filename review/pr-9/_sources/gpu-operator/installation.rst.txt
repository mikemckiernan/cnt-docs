...
Before installing the GPU Operator, ensure that the Kubernetes cluster meets the following prerequisites:

- Nodes must be configured with a container engine such as containerd or CRI-O.
- Node Feature Discovery (NFD) is a dependency for the Operator on each node.
  ...
  If NFD is already running in the cluster prior to the deployment of the operator, then the Operator can be configured not to install NFD.

Before installing the GPU Operator on NVIDIA vGPU, check the following prerequisites:
...
- The NVIDIA vGPU Host Driver version 12.0 (or later) is pre-installed on all hypervisors hosting NVIDIA vGPU accelerated Kubernetes worker node virtual machines.
  Refer to `NVIDIA vGPU Documentation <https://docs.nvidia.com/grid/12.0/index.html>`_ for details.
- An NVIDIA vGPU License Server is installed and reachable from all Kubernetes worker node virtual machines.
...
- Each Kubernetes worker node in the cluster has access to the private registry.
  ...
  See the Kubernetes documentation for more information about image pull secrets.
  You will need to provide these secrets to the NVIDIA GPU Operator in the driver section of the ``values.yaml`` file
  when you install the Operator.
- Git and Docker or Podman are required to build the vGPU driver image from source repository and push to the local registry.
