...
The mirror registry can be located on a jump host, with connectivity to both your workstation and the disconnected cluster, or a completely disconnected (airgapped) host, which requires removable media to physically move the mirrored content to the disconnected environment.

Below is the typical topology of a disconnected cluster.
...
* Download oc-mirror tools
  ...
  Copy the ``oc-mirror`` binary to the ``/usr/bin`` folder.
...
#. Export ``JUMP_HOST=ec2-3-16-218-255.us-east-2.compute.amazonaws.com``:
   ...
   .. note:: If you want to mirror other operators, you need to create different ``imageset-config.yaml`` files, using different metadata names. For example, NFD can use oc-mirror-nfd-metadata and GPU can use oc-mirror-gpu-metadata. Otherwise, the ``oc-mirror`` tool will overwrite previous mirrored images.
...
#. Create the files and folders using the following command:
...
#. Edit the ``imageset-config-gpu.yaml`` file as shown below:
...
#. If you only want to mirror 22.9.0, change the file as shown below:
...
#. Mirror GPU Operator CatalogSource using the following command:
